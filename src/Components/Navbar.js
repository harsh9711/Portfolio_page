...
const CustomLink = ({ href, title, className = "" }) => {
    ...
    return (
        <Link href={href} className={`${className} relative group`}>
            {title}
            <span className={`h-[1px] inline-block w-0 absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] bg-dark ease duration-300 ${router.asPath === href ? 'w-full' : 'w-0'} dark:bg-light`}>&nbsp;</span>
        </Link>
    );
};
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
    ...
    return (
        <button href={href} className={`${className} relative group text-light dark:text-dark my-2`} onClick={handleClick}>
            {title}
            <span className={`h-[1px] inline-block  bg-light w-0 absolute left-0 -bottom-0.5 group-hover:w-full transition-[width]  ease duration-300 ${router.asPath === href ? 'w-full' : 'w-0'} dark:bg-dark`}>&nbsp;</span>
        </button>
    );
};
...
const Navbar = () => {
    ...
    // Ensure correct initialization of mode state
    React.useEffect(() => {
        if (mode === "dark") {
    ...
    return (
        <>
            <header className='w-full px-32 py-8 font-medium flex justify-between items-center dark:text-light relative z-10 1g:px-16 md:px-12 sm:px-8'>
                ...
                
                {
                    isOpen?
                        <motion.div
                         ...
                         className="min-w-[70vw] flex flex-col justify-between z-30 items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-dark/90 dark:bg-light/75 rounded-1g backdrop-blur-md py-32">
