...
{title}
<span className={`h-[1px] inline-block w-0 absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] bg-dark ease duration-300 ${router.asPath===href?'w-full':'w-0'}`}>&nbsp;</span>
</Link>
)
}
const Navbar = () => {
  return (
    ...
    <header className='w-full px-32 py-8 font-medium flex justify-between items-center'>
          ...
          <nav className='flex items-center justify-between flex-wrap'>
              ...
              <motion.a href="/projects" target={"_blank"} 
                  whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.9 }}
                  className="w-6 mx-3"><LinkedInIcon /></motion.a>
              <motion.a href="/articles" target={"_blank"}   whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.9 }}
              ...
              <motion.a href="/articles" target={"_blank"}   whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.9 }}
                  className="w-6 ml-3"><SunIcon /></motion.a>
